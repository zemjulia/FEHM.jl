...
Command to run: julia path/to/directory/tec2vtk.jl --arguments       
...
function tec_to_df(f, variables=0)
	...
	#d = readdlm(fh)
	close(fh)
	df = DataFrames.DataFramd(d, :auto)
	...
	for v in names(df)
		...
		if in(v, names(diff_df))
			if length(var_diff) > 0
				...
				vtk_point_data(vtkfile, df[!m v] - diff_df[!, v], string("Delta ", v))
			end
		end
	end
...
if (length(fs) > 0)
	...
	# Collect variables from the first file 
